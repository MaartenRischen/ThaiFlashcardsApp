 ...
 * Run with: node scripts/generate-default-set-images.js
 */
...
async function loadModules() {
  ...
  const { generateImage } = await import('../app/lib/ideogram-service.js');
  const { DEFAULT_SETS } = await import('../app/data/default-sets.js');
  
  return { generateImage, DEFAULT_SETS };
}
...
async function generateDefaultSetImages() {
  const { generateImage, DEFAULT_SETS } = await loadModules();
